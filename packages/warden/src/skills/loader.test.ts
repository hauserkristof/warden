...
const fixtureSkillsDir = join(monorepoRoot, '.agents', 'skills');
...
describe('resolveSkillAsync', () => {
  it('resolves skills from conventional directories', async () => {
    const skill = await resolveSkillAsync('testing-guidelines', monorepoRoot);
...
describe('direct path resolution', () => {
  ...
  it('resolves relative path with repoRoot', async () => {
    const skill = await resolveSkillAsync('./.agents/skills/testing-guidelines', monorepoRoot);
    expect(skill.name).toBe('testing-guidelines');
  });
...
describe('BUILTIN_SKILL_DIRECTORIES', () => {
  ...
  it('documents the built-in skill names trusted by fallback resolution', () => {
  ...
  it('includes the monorepo package root as an action-bundle fallback', () => {
  ...
  it('does not trust cwd package-shaped paths as new built-in skill names', async () => {
    ...
    try {
      const fakeSkillDir = join(repoRoot, 'packages', 'warden', 'src', 'builtin-skills', 'attacker-skill');
...
`,
      ...
      expect(isBuiltinSkillName('attacker-skill')).toBe(false);
      expect((await discoverAllSkills()).has('attacker-skill')).toBe(false);
      await expect(resolveSkillAsync('attacker-skill')).rejects.toThrow('Skill not found');
    } finally {
      process.chdir(cwd);
      rmSync(repoRoot, { recursive: true, force: true });
    }
...
`
  ...
  it('loadSkillFromFile accepts .md extension', async () => {
    ...
    const skillMdPath = join(fixtureSkillsDir, 'testing-guidelines', 'SKILL.md');
  ...
  it('loadSkillsFromDirectory returns entry paths for tracking', async () => {
    clearSkillsCache();
    const skills = await loadSkillsFromDirectory(fixtureSkillsDir);
